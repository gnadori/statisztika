...
Ez az útmutató elmagyarázza, hogyan használd a student_analysis.R szkriptet, és mit keress az eredményekben.
Hogyan használd
Fájlok mentése: Helyezd a student_analysis.R és az 1zt.csv fájlokat ugyanabba a mappába a számítógépeden.
R megnyitása: Nyisd meg az R környezetet (például az RStudio-t).
Munkakönyvtár beállítása: Állítsd be az R munkamenet munkakönyvtárát (working directory) arra a mappára, ahová a fájlokat mentetted.
RStudio-ban ezt könnyen megteheted: Session -> Set Working Directory -> To Source File Location.
Csomagok telepítése (Csak először):
A student_analysis.R szkriptben keresd meg az install.packages(...) kezdetű sorokat.
...
Szkript futtatása: Futtasd a teljes student_analysis.R szkriptet. Az eredmények és az ábrák meg fognak jelenni a konzolon és az ábrák panelen (Plots pane).
...
Amikor kiíratod a reliability_results-t, sok szöveget látsz majd. Erre a sorra koncentrálj:
raw_alpha (Cronbach-alfa): Ez a te teszt-megbízhatósági pontszámod. Megmondja, hogy a teszted következetesen egyetlen fogalmat mér-e.
> 0.9: Kiváló
